...
const Logo = () => (
  <a href="/" style={{ display: 'block', width: '3.25rem', flexShrink: 0 }}>
    <svg viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '3.25rem', height: '52px', display: 'block' }}>
      <path d="M5.378 0h25.2C33.528 0 36 2.435 36 5.348v25.26C36 33.566 33.528 36 30.578 36h-25.2C2.428 36 0 33.565 0 30.609V5.348C0 2.435 2.429 0 5.378 0Z" fill="#A6CE39"/>
      <path d="M21.774 17.826c-.564-1.392-1.388-2.565-2.516-3.435-1.127-.87-2.428-1.304-3.99-1.304-1.128 0-2.169.217-3.123.652-.954.435-1.778 1.13-2.299 1.956h-.043V4.521H5.119v26.913s4.337.522 4.467-2.26V28.739l.043.217c.087.218.26.435.521.826.347.392.91.826 1.778 1.218.911.391 1.995.565 3.427.565 1.561 0 2.862-.391 3.99-1.26 1.085-.914 1.952-2 2.516-3.436.564-1.478.867-2.956.867-4.478-.087-1.652-.39-3.13-.954-4.565Zm-5.075 8.478c-.65 1.13-1.605 1.609-2.906 1.609-.91 0-1.648-.261-2.298-.74-.651-.565-1.171-1.26-1.475-2.13-.304-.87-.52-1.782-.52-2.739 0-.956.216-1.913.52-2.782a4.585 4.585 0 0 1 1.475-2.087c.65-.566 1.388-.827 2.298-.827.868 0 1.605.261 2.212.74.564.521.998 1.217 1.302 2.087.26.87.39 1.782.39 2.826.043 1.652-.347 2.956-.998 4.043Z" fill="#ED1C24"/>
      <path d="M15.96 4.522h2.862v8.609c-.867-.566-1.822-.87-2.862-.87V4.522Zm4.511 9.217c3.774-1.782 8.068.175 9.586 4.087a17.23 17.23 0 0 1 .954 4.435c0 1.522-.303 3-.867 4.479-.564 1.434-1.432 2.521-2.516 3.434-1.128.87-2.429 1.261-3.99 1.261-1.432 0-2.516-.174-3.427-.565a7.049 7.049 0 0 1-.564-.26c.65-.523 1.215-1.175 1.735-1.87.477.174 1.041.26 1.648.26 3.557 0 4.728-3.782 4.728-6.739 0-2.913-1.084-6.782-4.727-6.782-.521 0-1.041.087-1.519.26-.433-.652-.954-1.217-1.518-1.739.13-.086.304-.174.477-.26Z" fill="#000"/>
    </svg>
  </a>
)
...
export function CheckoutHeader({ step, selectedAddress, selectedSlot, onChangeAddress, onChangeSlot }) {
...
// ─── Checkout Page (Step 1 — address + slot) ─────────────────────────────────
export default function CheckoutPage({ addresses, selectedAddressId, selectedSlot, onSelectAddress, onSelectSlot, onUseCurrentLocation, onAddAddress, onCancel, onConfirm, checkoutLoading, cart }) {
  ...
  const handleAddAddress = () => {
    ...
    onAddAddress({ id: `addr-${Date.now()}`, label: newAddress.label || 'Home', title: newAddress.label || 'Home', address: `${newAddress.line1}, ${newAddress.city}, ${newAddress.state}, ${newAddress.postal}`, details: 'Custom address' })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f6f0', fontFamily: 'ProximaNova,Helvetica,Arial,sans-serif' }}>
      <CheckoutHeader step={1} selectedAddress={selectedAddress} selectedSlot={selectedSlot} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '8px', border: '0.8px solid #eee', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: DARK }}>Add new address</h2>
              <button type="button" onClick={() => setShowAddForm(v => !v)} style={{ fontSize: '13px', color: GREEN, fontWeight: 600, background: 'none', border: `1px solid ${GREEN}`, borderRadius: '4px', padding: '4px 12px', cursor: 'pointer' }}>
                + Add new
              </button>
            </div>
            ...
            {showAddForm && (
              <div style={{ padding: '16px 24px', background: '#f8faf4', borderBottom: '1px solid #eee' }}>
                <div style={{ display: 'grid', gap: '10px' }}>
                  {[['label','Label (Home / Office)'],['line1','Address line'],['city','City'],['state','State'],['postal','PIN / ZIP']].map(([name,ph]) => (
                    <input key={name} value={newAddress[name]} onChange={e => setNewAddress(p => ({...p,[name]:e.target.value}))} placeholder={ph} style={inp} />
                  ))}
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <button type="button" onClick={handleAddAddress} style={primaryBtn}>Save address</button>
                    <button type="button" onClick={() => setShowAddForm(false)} style={ghostBtn}>Cancel</button>
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{ padding: '16px 24px', display: 'grid', gap: '12px' }}>
              {addresses.map(addr => (
                <button key={addr.id} type="button" onClick={() => onSelectAddress(addr.id)} style={{ textAlign: 'left', padding: '16px', borderRadius: '10px', cursor: 'pointer', border: addr.id === selectedAddressId ? `2px solid ${GREEN}` : '1px solid #ddd', background: addr.id === selectedAddressId ? 'rgba(94,148,0,.06)' : '#fafafa', transition: 'all .18s' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
                    <span style={{ fontSize: '11px', fontWeight: 700, padding: '2px 8px', borderRadius: '20px', background: addr.id === selectedAddressId ? GREEN : '#eee', color: addr.id === selectedAddressId ? '#fff' : '#555', textTransform: 'uppercase' }}>{addr.label || 'Address'}</span>
                    {addr.id === selectedAddressId && <span style={{ marginLeft: 'auto', color: GREEN, fontSize: '12px', fontWeight: 700 }}>✓ Selected</span>}
                  </div>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: DARK, marginBottom: '4px' }}>{addr.title}</div>
                  <div style={{ fontSize: '13px', color: '#666', lineHeight: 1.5 }}>{addr.address}</div>
                  {addr.details && <div style={{ marginTop: '6px', fontSize: '12px', color: '#999' }}>{addr.details}</div>}
                </button>
              ))}
            ...
            <div style={{ maxHeight: '240px', overflowY: 'auto', display: 'grid', gap: '10px', marginBottom: '12px', padding: '4px' }}>
              {cart.map(item => (
                <div key={item.product_id || item.id} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  <div style={{ fontSize: '14px', fontWeight: 700, flexShrink: 0 }}>₹{((item.product?.price || 0) * item.quantity).toFixed(2)}</div>
                </div>
              ))}
            </div>
            <div style={{ paddingTop: '10px', paddingBottom: '10px', paddingLeft: '8px', paddingRight: '8px', justifyContent: 'space-between', display: 'flex', borderTop: '0.8px solid #eee' }}>
              <span style={{ fontSize: '16px', color: DARK, fontWeight: 600 }}>Total Amount Payable</span>
              <span style={{ fontSize: '16px', color: DARK, fontWeight: 600 }}>₹{subtotal.toFixed(2)}</span>
            </div>
            {savings > 0 && (
